...
func LastCheckpoint(dir string) (string, int, error) {
	...
	for i := len(files) - 1; i >= 0; i-- {
		...
		return filepath.Join(dir, fi.Name()), k, nil
	}
...
// Checkpoint creates a compacted checkpoint of segments in the given WAL.
...
func Checkpoint(w *wal.WAL, m, n int, keep func(id uint64) bool, mint int64) (*CheckpointStats, error) {
	...
	var sr io.Reader
	{
		lastDir, k, err := LastCheckpoint(w.Dir())
		...
		if err == nil {
			...
			last, err := wal.NewSegmentsReader(lastDir)
			if err != nil {
	...
	if err := w.Truncate(n + 1); err != nil {
		return nil, errors.Wrap(err, "truncate obsolete segments")
	}
	if err := DeleteCheckpoints(w.Dir(), n); err != nil {
		return nil, errors.Wrap(err, "delete old checkpoints")
	}
	return stats, nil
}
