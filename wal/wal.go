...
func NewSize(logger log.Logger, reg prometheus.Registerer, dir string, segmentSize int) (*WAL, error) {
	...
	if j == -1 {
		w.segment, err = CreateSegment(w.dir, 0)
	} else {
		w.segment, err = OpenWriteSegment(w.dir, j)
	}
	if err != nil {
		return nil, err
	}
...
const (
	...
	recSegmentTerm recType = 6 // marker for segment termination page
)
...
func (r *Reader) next() (err error) {
	...
	for {
		...
		// to determine valid record sequences.
		i++
	}
}
