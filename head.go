...
type headMetrics struct {
	...
	walCheckpointDuration prometheus.Summary
}
...
func newHeadMetrics(h *Head, r prometheus.Registerer) *headMetrics {
	...
	m.walCheckpointDuration = prometheus.NewSummary(prometheus.SummaryOpts{
		Name: "prometheus_tsdb_wal_checkpoint_duration_seconds",
		Help: "Duration of WAL checkpointing.",
	})
...
func (h *Head) backfill(r *wal.Reader) error {
	mint := h.MinTime()
...
func (h *Head) Init() error {
	...
	if err == nil {
		sr, err := wal.NewSegmentsReader(cp)
		if err != nil {
		...
		if err := h.backfill(wal.NewReader(sr)); err != nil {
	...
	err = h.backfill(wal.NewReader(sr))
	if err == nil {
		return nil
	}
...
func (h *Head) Truncate(mint int64) error {
	...
	h.metrics.walCheckpointDuration.Observe(time.Since(start).Seconds())
